...
impl IntrinsicGuacClient {
    ...
    pub async fn certify_vex_statement(&self, certify_vex_statement_spec: &CertifyVexStatementSpec) -> Result<Vec<CertifyVexStatement>, Error> {
        ...
        let response_body =
            post_graphql::<QueryCertifyVexStatement, _>(self.client(), self.url(), variables).await?;
